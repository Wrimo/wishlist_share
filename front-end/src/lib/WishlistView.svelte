<script>
    ...
    onMount(async () => {
        ...
        if (Object.keys($selectedPerson).length === 0) {
            // if the page has been refreshed, go back home
    ...
    async function toggledPurchased(id) {
        const index = data.findIndex(x => x["id"] === id); 
        data[index]["purchased"] = !data[index]["purchased"];
        const response = await fetch(
            `http://127.0.0.1:8000/wishlist_item/${id}`,
            { method: "POST" },
        );
...
<div class="h-screen w-full flex justify-center items-center">
    ...
    >
        ...
        {#if data.length === 0}
            <p class="p-2 text-black text-2xl col-span-12 m-auto">
                This person hasn't filled out their wishlist yet!
            </p>
        {:else}
            {#each data as item}
                <button
                    class="col-span-12 py-1 px-4 rounded-xl bg-slate-300 text-left text-black text-xl"
                    class:bg-emerald-400={item["purchased"]}
                    class:bg-slate-300={!item["purchased"]}
                    on:click={toggledPurchased(item["id"])}
                >
                    {item["name"]}
